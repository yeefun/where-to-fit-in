<template lang="pug">
  section.reports
    article.report(v-for="(report, idx) in reports" :key="report.key" :id="report.key" v-if="!currentReportIdx || currentReportIdx === idx")
      .full-page.full-img.report__cover(:id="`${report.key}__cover`")
      h1 {{report.title}}
      .report__preface
