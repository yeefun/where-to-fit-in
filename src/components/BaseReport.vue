    ...
    article.rpt(
      ...
      .rpt__cover-txt(:style="{ color: (rpt.id === 5 || rpt.id === 2) ? '#f7f7f7' : '#1b2733'}")
        ...
        .rpt__intro(v-if="!isRptContent" :style="{ marginTop: $root.inRptCover ? '32px' : '24px'}")
          ...
          .rpt__intro--related(v-if="!$root.inRptCover && !isRptContent")
            p {{rpt.introRelated}}
      component(:is="`ReportContent${rpt.id}`" v-if="isRptContent")
</template>
...
export default {
  ...
  created () {
    ...
    if (id) {
      for (let i = 1; i <= 4; i++) {
        const idx = id + i
        this.relatedRpts.push(this.allRpts[`rpt${idx > 5 ? (idx - 5) : idx}`])
      }
    } else {
      this.relatedRpts = this.allRpts
    }
  },
  data () {
    return {
  ...
  methods: {
    isRpt (id) {
      return !this.curtRptId || (this.curtRptId === id)
    },
    showRptFromHome (id) {
      this.$root.curtRpt = document.getElementById(`rpt${id}`)
